...
{
    assert(has_ber_extension("validmap.ber") == 1);
...
{
    int width, height;
    assert(get_matrix_dimensions(&width, &height, "validmap.ber") == 0);
    assert(width == 13);
    assert(height == 5);
    assert(get_matrix_dimensions(&width, &height, "invalid_map.ber") == -1);
